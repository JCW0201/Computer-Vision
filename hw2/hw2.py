...
if __name__ == '__main__':
    ...
    img_path = os.path.join(base_dir, 'original.jpg')

    print("目前執行檔案 =", __file__)
    print("base_dir =", base_dir)
    print("img_path =", img_path)
    print("圖片存在嗎？", os.path.exists(img_path))
...
# i. Gaussian smooth results: 𝜎=5 and kernel size=5 and 10 (2 images)
    ...
    img_filtered_K5 =  img_filtered_K5 / np.amax(img_filtered_K5) * 255 #找最大值，將整張圖的像素值縮放到0~255之間
...
# (2) window size = 30x30 (1 image)
    ...
    harrisim=structure_tensor(gradient_magnitude_K10, gradient_direction_K10, k, sigma) #Ix, Iy傳入structure_tensor，避免還原Ix, Iy的過程失真
    window_size=3
